...
my $self = shift(@_);
my $id = shift(@_);
  my %args = $self->id2fileser(id => $id);
  my $qstr1 = "select locus_tag, product, start_pos, end_pos, strand from features where filename = '$args{filename}' and serial = $args{serial}";
  my ($locus_tag, $product, $start_pos, $end_pos, $strand) = $handle->selectrow_array($qstr1);
  my $qstr2 = "select aa_seq, ttapos from proteins where filename = '$args{filename}' and serial = $args{serial}";
  my ($aa_seq, $ttapos) = $handle->selectrow_array($qstr2);
  my $qstr3 = "select organism, porg, dorf from orgtab where filename = '$args{filename}'";
  my ($organism, $porg, $dorf) = $handle->selectrow_array($qstr3);
  return(aa_seq => $aa_seq, locus_tag => $locus_tag,
  organism => $organism, product => $product, porg => $porg, dorf => $dorf,
  start_pos => $start_pos, end_pos => $end_pos, strand => $strand, ttapos => $ttapos
  );
}
# }}}
